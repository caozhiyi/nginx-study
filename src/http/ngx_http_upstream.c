...
ngx_http_upstream_connect(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    // 调用 ngx_event_connect_peer 方法主动向上游服务器发起连接请求，
    // 需要注意的是该方法已经将相应的套接字注册到epoll事件机制来监听读、写事件；
    ...
    ngx_uint_t do_write)
{
    ...
    /*
    * 把写事件的回调方法设置为ngx_http_upstream_dumy_handler方法(不进行任何实际操作)；
    */
    ...
    ngx_http_upstream_t *u)
{
    ...
    c->log->action = "reading upstream";
    // 若上游连接上读事件的 timedout 标志位为 1，
    // 表示读事件已经超时，则设置 upstream_error 为 1，调用 ngx_connection_error 方法接收当前函数；
    if (rev->timedout) {
        ...
        // 若上游连接上读事件的 timedout 标志位为 0，
        // 则调用 ngx_event_pipe 方法接收上游响应包体；
        if (rev->delayed) {
